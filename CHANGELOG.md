...
驗證：self-test ALL PASS；offscreen 出圖確認標籤離開線與點。`py_compile` 通過。不動 pipeline 輸出。

檔案：`ClosureTemperature.py`（段速率 annotate offset + bbox halo）；`.work/.app_info.txt` 3.9.3 → 3.9.4。
...
### 根因
Plot Controls 在 Summary 頁,diagram 大圖在各自分頁。Apply → `_refresh_diagrams` 重生 PNG + `_reload_all_pngs`,但此時 diagram 分頁**還沒顯示**,大圖 label 的 `lbl.size()` 是還沒佈局的尺寸(甚至 0),`pm.scaled(lbl.size())` 把圖縮到錯誤/極小尺寸。切到該分頁時 label 才佈局成正常大小,但 pixmap 已是壞掉的縮放,且沒有任何 re-fit。

### 修法
bottom `QTabWidget.currentChanged` 接 deferred `_reload_all_pngs`(`QTimer.singleShot(0, ...)`):切到任一分頁時,等該頁佈局到真實尺寸後重新載入 + 縮放 PNG。→ 從 Summary 改完 Apply,切到 diagram 分頁就會看到剛重生、且正確縮放的圖。deferred 是為了讓佈局先完成,`lbl.size()` 才是對的。

### 影響
- 純顯示刷新時機,**不動計算**。每次切分頁多載一次 PNG(從硬碟,很快)。

### 檔案改動
- `AutoPipeline.py`:`self._tabs.currentChanged` → deferred `_reload_all_pngs`。
- `.work/.app_info.txt`:3.8.87 → 3.8.88

---

## V3.8.87（2026-06-14）— ³⁶Ar blank → Inverse Isochron 敏感度（#42）

使用者澄清:之前一直要的「inverse isochron 敏感度」是指 **³⁶Ar blank 對 inverse isochron 的敏感度**(拉 ³⁶ blank,看 inverse isochron 的點、trapped ⁴⁰/³⁶、age 怎麼動)。

### 做法
- 既有的「³⁶Ar blank → Age Spectrum 敏感度」對話框(滑桿拉 k)頂端加 **檢視 combo:Age Spectrum / Inverse Isochron**。
- 切到 Inverse Isochron:畫 ³⁶Ar/⁴⁰Ar vs ³⁹Ar/⁴⁰Ar,每點標**溫度**(找哪些溫階共線),baseline(k=1)淡色墊底、目前 k 藍色;下方讀數即時報 **trapped ⁴⁰/³⁶ = 1/截距** 與 **age(F=−slope/intercept)**。
- 物理:拉 ³⁶ blank ×k → 量測 ³⁶(ar[0])平移 −(k−1)·³⁶blank₀,³⁹_m/⁴⁰_m 不變,所以點**垂直滑動**,trapped 跟著變。⁴⁰_m(ar[22])、³⁹_m(ar[16])不受 ³⁶ blank 影響。
- 回歸線用 `np.polyfit`(快速 OLS)畫趨勢,**僅供 what-if 預覽**(主 isochron 仍是 York);不改任何實際科學輸出。

### 驗證（headless）
- k=1 重現量測 ³⁹/⁴⁰、³⁶/⁴⁰;構造 trapped=298.56 的合成資料 → 讀數 298.6;k=1.5(多扣 blank)→ trapped 升到 403(少 ³⁶ → ⁴⁰/³⁶ 變大,物理正確)。`py_compile` 過。

### 檔案改動
- `AutoPipeline.py`:`_ar36_scaled_isochron(k)`、`_draw_ar36_inverse(ax,k)`、`_show_ar36_spectrum_dialog` 加檢視 combo + `_redraw` 分支。
- `.work/.app_info.txt`:3.8.86 → 3.8.87

---

## V3.8.86（2026-06-14）— 從 Parameter 頁可直接回 Argon Pipeline（不丟 session）

使用者回報:v3.8.85 從 AutoPipeline 按 Parameter 切到參數頁後,**沒辦法直接切回 Argon Pipeline**,只能 Return 回首頁,再進 pipeline 要重載、原本 session 不見。

### 根因 / 釐清
其實 AutoPipeline 的頁面物件不會被銷毀,session(載入的 .dat、T₀、算好的年代)一直都在;問題只是**少了回去的路**:參數頁的 Return 一律回首頁。

### 修法
- `toPS_from_pipeline`(從 AutoPipeline 進參數頁)設旗標 `_ps_from_ap=True`;正常從首頁/選單進的 `toPS` 設回 `False`。
- 參數頁 Return(`return_2`)改接新的 `PS_return()`:若 `_ps_from_ap` → 回 **Argon Pipeline**(`toAP()`,會 showMaximized 還原 + `set_context` 重推參數,**session 保留**因為頁面物件沒被砍);否則照舊回首頁。
- 附帶好處:在參數頁改了 J 等參數並 Save 後 Return,`toAP` 的 `set_context` 會把新參數推進 AutoPipeline(下次重跑生效)。

### 檔案改動
- `NTNU_DataReduction.py`:`_ps_from_ap` 旗標、`PS_return()`、`toPS` 重置旗標、`toPS_from_pipeline` 設旗標、`ParameterSettingPage.return_2` 改接 `PS_return`。
- `.work/.app_info.txt`:3.8.85 → 3.8.86
...
## V3.8.84（2026-06-14）— AgeCalc 可編輯 J value + 真重算（#40 + #37）

使用者要求:AutoPipeline 頁面要能調常數,**重點是 J value**;順帶修好 #37 那顆一直是 no-op 的 Recalculate。

### 做法（重算走標準 pipeline,不是 closed-form 近似）
- AgeCalc 控制列 atm 旁邊加 **J 輸入框**,pipeline 跑完自動帶入目前 J(任一階 `ar[44]`)。
- 按 **Recalculate**:讀 J 輸入框 + atm 輸入框,丟給 parent(`AutoPipelineWindow._recalc_with_params`),把 `J value` /(atm)`Atmospheric Ratio 40/36(a)` 換進 params,**重跑整條 pipeline**。
- 為什麼重跑整條而不是只改顯示:`calcAge(mr_csv, J, Js, Ji, consts)` 本來就吃 J,重跑讓**表格 / banner / 圖 / datum / export 全部用新 J 一致更新**,不會有「表格新 J、isochron 圖舊 J」的矛盾。代價是會連 T₀/MassRatio 一起重算(那兩個跟 J 無關,稍慢但安全、且重用已驗證路徑)。

### 重要行為
- **J 不變時重跑 = 完全相同結果**(同一條 pipeline、同 params),結構上保證,不需擔心數值漂移。
- 重跑會**重置 plateau 勾選**(populate 把 mask 設回全選),改完 J 要重按 Auto plateau 或重新勾。
- 改 J = 改科學輸出。**請用 NO.65 確認:同 J 重跑仍 9.77;改 J 年代依 `t=ln(1+J·F)/λ` 等比例移動。**

### 尚未做（v2 候選）
- 只重算 age-calc 段(跳過 T₀/MR)以加速;full constants editor 對話框(λ、production ratios 等,目前只開放 J + atm)。

### 檔案改動
- `AutoPipeline.py`:AgeCalc `jEdit` 欄 + populate 帶入、`_recalculate_with_atm` 改讀 J、parent `_recalc_with_params`(換 J/atm param → 重跑)、agePage 建立後接 `_on_recalc_request`。
- `.work/.app_info.txt`:3.8.83 → 3.8.84

---

## V3.8.83（2026-06-14）— 啟動 loading 畫面馬上出現（splash-first）

使用者回報:「開啟程式後 loading 沒有馬上出現,每次都是 cmd 跑一段時間後才跳出來,不符合開啟儀式」。

### 根因
splash 是在 `App.__init__` 裡建的,而那是在**所有重 import 之後**才執行的。module 載入順序:numpy → PyQt5 → ... → Utilities(拉 pandas/matplotlib) → AutoPipeline,光這些冷啟動就好幾秒,期間使用者只看到黑 cmd,splash 等 import 全跑完才冒出來。

### 修法（splash-first）
- 在 `NTNU_DataReduction.py` **最上面**(只 import sys/os/PyQt5 之後,numpy 等重 import 之前)先建 QApplication + 顯示 splash(`_BOOT_SPLASH`),`processEvents()` 畫出來,再繼續跑重 import。→ splash 在 ~0.15s 內就出現,那幾秒的 import 是在 splash 已經亮著的狀態下跑。
- `App` 不再自己 new QApplication,改 `QApplication.instance()` 重用;splash 區塊重用 `_BOOT_SPLASH`,只把版本/日期/Loading 疊字的 pixmap `setPixmap` 換上去。`_splash_t0` 沿用 boot 時間,所以 min-display(v3.8.81 的 1000ms)早就滿足,主視窗一好就收 splash,不額外等。

### 影響
- **開啟瞬間就看到 loading 卡片**(logo),import 在背後跑,符合開啟儀式。總時間沒變短(import 還是要那幾秒),但**從「黑畫面等很久」變成「splash 馬上亮、背後載入」**。
- 純啟動序列,**不動任何數學/科學輸出**。
- ⚠️ 啟動關鍵改動(動到 module 最上層 + QApplication 建立),**請務必確認程式還能正常開啟**。

### 檔案改動
- `NTNU_DataReduction.py`:top-of-module `_BOOT_APP`/`_BOOT_SPLASH`/`_BOOT_T0` 預啟動 splash、`App` QApplication 改重用 instance、splash 區塊重用 boot splash。
- `.work/.app_info.txt`:3.8.82 → 3.8.83
...
## V3.8.81（2026-06-12）— 啟動加速:移除 sklearn + 縮短 splash 下限

使用者回報開啟變很久,目標 5 秒內。量測 import 成本後修兩處,**不動任何數學**。

### 移除 sklearn（最大宗,冷啟動 ~7.5s）
- 全 repo `sklearn` 只用在一個函式 `r2_score`(`Utilities.py`、`AutoPipeline.py` 各一處 import,共 8 個呼叫)。但 `import sklearn` 冷啟動要 ~7.5s(它載一大堆子模組檔案)。
- 改成 `Utilities.py` 內自寫 `r2_score`(numpy 三行:1 − SS_res/SS_tot,含 ss_tot=0 的 sklearn 邊界行為),`AutoPipeline.py` 改 `from Utilities import r2_score`。
- **驗證 bit-identical**:2000 組隨機 + 邊界(perfect fit)對照 `sklearn.metrics.r2_score`,max |diff| = 0.0。fit-quality R² 完全沒變 → fit 選擇 / T₀ 不受影響,**不需重跑 NO.65**。確認 `import Utilities` 後 `sklearn` 不在 `sys.modules`。

### splash 下限 3000 → 1000 ms（暖啟動瓶頸）
- 量測:暖啟動(OS 檔案快取熱)整條 import 鏈 ~1.1s,所以舊的「splash 最少顯示 3 秒」其實是暖啟動的主要瓶頸。
- 縮到 1000ms,夠看版本/credits 又不卡住主視窗。

### 影響
- **冷啟動**(剛開機/快取冷):少掉 sklearn 約 7.5s。
- **暖啟動**:splash 下限少 2s → 約 3-4s 進主視窗。
- 純啟動效能,數學/科學輸出零變動。seaborn 保留(AutoPipeline 繪圖風格依賴 `sns.set()`,見 AutoPipeline.py:1705)。

### 檔案改動
- `Utilities.py`:自寫 `r2_score`(取代 sklearn import)。
- `AutoPipeline.py`:`from Utilities import r2_score`。
- `NTNU_DataReduction.py`:`_splash_min_ms` 3000 → 1000。
- `.work/.app_info.txt`:3.8.80 → 3.8.81

---

## V3.8.80（2026-06-12）— Plot Controls X/Y 顯示目前 min/max（不再是 0）（#41）

使用者回報:Plot Controls 選「Apply to 特定 diagram」時,X/Y min/max 欄顯示 0,不是該圖目前的最大最小值。

### 根因
diagram 的真實 rendered 軸範圍存在 `self._actual_xlims/_actual_ylims`,只有 `_refresh_diagrams()` 跑過才會填。但 pipeline 跑完是 worker 預先生成 PNG,`populate()` 結尾只呼叫 `_reload_all_pngs()`（純載入圖檔）,**不跑 `_refresh_diagrams()`**。所以一份新資料載進來後 `_actual_xlims` 是空的,`_plot_target_changed` 讀 `ax.get(key)` 取不到 → spinbox 停在預設 0。

### 修法
- 新增 `_prime_axis_limits()`:每份資料 prime 一次（`_limits_primed` guard 防重複 re-render）,跑一趟 `_refresh_diagrams()` 把 `_actual_xlims/_ylims` 填滿並透過既有 `_sync_axis_controls_from_actual()` 把真實範圍寫回 spinbox。
- `populate()` 結尾 `QtCore.QTimer.singleShot(0, self._prime_axis_limits)` 延遲呼叫:表格/PNG 先顯示,prime 在事件迴圈下一拍跑,使用者開 Plot Controls 時 X/Y 已是真實 min/max。
- 每份新資料 `_limits_primed=False` 重置,所以換樣品會重新 prime。

### 驗證（headless）
- `py_compile` 過。邏輯:prime 一次後 `_actual_xlims` 有值 → `_plot_target_changed`/`_sync_axis_controls_from_actual` 既有路徑把值寫進 spinbox。純顯示層,不改任何科學數值。
- GUI 待使用者確認:載 NO.65 → 開 Plot Controls → 選某 diagram → X/Y 欄應顯示該圖現值。

### 檔案改動
- `AutoPipeline.py`:新 `_prime_axis_limits`、`populate()` 結尾延遲 prime。
- `.work/.app_info.txt`:3.8.79 → 3.8.80

---

## V3.8.79（2026-06-12）— AgeCalc 年代結果一律 2σ + Copy table（#38 + #35）

使用者：「1σ 不用了,我這邊計算都用 2σ」。把 AgeCalc 的**年代結果**顯示與輸出一律 2σ,並加 Summary 表複製。

### 2σ everywhere（#38,改成不用 toggle,直接 2σ）
- 新增 module-method `_pm(val, sig, prec)` → `'val ± 2σ'`。**×2 只在這一個 display helper 發生**;所有儲存的 sigma 與內部數學（plateau MSWD、v3.8.78 inverse 裁判 concordance 的 2·√(σ²) 帶）全部維持 1σ,只有顯示當下 ×2。避免污染統計。
- 套用點（全部走 `_pm` 或就地 ×2）：Summary 表 σ 欄 + 表頭「±2σ」、Total Fusion / Weighted Plateau（標「2σ」）/ Normal iso（含 trapped σ）/ Inverse iso banner、DFW/DFN/DFI 右側面板年代、³⁶Ar 敏感度對話框 plateau 與 baseline、AgeCalc_summary.csv（表頭「±2σ」+ 值 ×2）。
- 控制列加一條灰字「不確定度皆 2σ」全域註記。
- **J value（J ± σ_J）維持原樣**：那是輸入常數的給定誤差,不是年代結果,不 ×2。
- **Datum Publication（88 欄,`_build_datum_row`）維持 1σ**：它 byte-align NTNU/ArArCALC datum 格式（v3.8.24）,該格式的 component 誤差慣例就是 1σ,改了會破壞對齊也誤導。年代結果（banner/表/CSV）才 2σ。

### Copy table（#35 安全項）
- Summary 表上方控制列加「Copy table」:把整張表（含表頭）組成 TSV 丟剪貼簿,可直接貼進 Excel。按完短暫顯示「Copied ✓」。純讀可見表格,不重算。
- #35 的 single-line stat cells **取消**（使用者同意;且與 v3.8.78 加長的 DFI 面板方向矛盾）。

### 驗證（headless）
- `_pm`:1σ 0.14 → 顯示 0.280（2σ）、prec=4 → 0.2800、SYL31 1.95→3.900,中心值不變。`py_compile` 過。
- ⚠️ 純顯示/格式層,科學中心值與內部統計不變。但你對照 NO.65（9.77 ± 0.28）時要注意：**那個目標值若是 1σ,現在畫面顯示的會是它的 2 倍**;若目標本身是 2σ 就直接對得上。

### 檔案改動
- `AutoPipeline.py`:`_pm` / `_copy_summary` 新 method、Copy table 按鈕 + 2σ 全域註記、~12 處 σ 顯示點改 2σ、表頭與 export 表頭「±2σ」。
- `.work/.app_info.txt`:3.8.78 → 3.8.79

---

## V3.8.78（2026-06-11）— MSWD 顏色提示 + Inverse isochron 裁判 readout（#35/#39）

AgeCalc 右側資訊面板兩個可讀性 / 判讀升級，純顯示層、不動任何科學計算：

### MSWD 顏色提示（#35 一部分）
Age Spectrum / Normal / Inverse 三個面板的 MSWD 文字依統計門檻上色，不用再心算「這個 MSWD 算不算過大」：
- 門檻 `hi = 1 + 2·√(2/(n−2))`（Wendt-Carl 1991 的 √MSWD 1σ 信賴帶上緣近似）。
- `MSWD ≤ hi` → 綠（OK）；`≤ 2·hi` → 琥珀（high）；更大 → 紅（excess scatter）。n<3 或無值 → 灰。
- 共用 module helper `_mswd_verdict(mswd, n)`，面板用 `_mswd_line()` 包成帶色 span。

### Inverse isochron 裁判 readout（#39 一部分）
Inverse 面板在 isochron age / MSWD / trapped 之下新增兩行判讀：
- **trapped ⁴⁰/³⁶ vs 大氣（298.56）**：290–307 綠「air-like」；>307 琥珀「trapped > air → excess Ar?」；<290 紅「unphysical → check」。直接點出馬遠溪那類 excess Ar 訊號。
- **plateau 一致性**：有選 plateau 時比 `|age_pl − age_iv|` 與 `2·√(σ_pl² + σ_iv²)`，落在 2σ 內 → 綠「plateau 一致」，否則紅「與 plateau 不一致」。plateau↔isochron concordance 是 Ar/Ar 數據可信度的標準檢查。

### 驗證（headless 邏輯測試）
- `_mswd_verdict` 門檻：n=16 → hi=1.756；MSWD 1.0/1.7→OK、2.5→high、5.0→excess；None/n<3→灰。全過。
- 裁判分支：trapped 298.6+concordant→綠/一致；450→excess；150→unphysical；age 差很大→不一致；無 plateau→concordance 略過。全過。
- `py_compile` 過。純顯示層，科學數值不變，不需重跑 NO.65（但累積的 Ca/K、Total Fusion、York 預設、plateau 選擇仍建議一次 NO.65 對照）。

### 檔案改動
- `AutoPipeline.py`：module helper `_mswd_verdict`；`_update_diagram_info` 內 `_mswd_line()` + DFW/DFN/DFI 面板上色 + DFI 裁判 readout。
- `.work/.app_info.txt`：3.8.77 → 3.8.78
...
## V3.8.74（2026-06-05，影響科學輸出）— 修 AgeCalc 讀 calcAge return 的 stale index（⁴⁰Ar(r)% / isochron / Ca/K）

使用者發現 ⁴⁰Ar(r)% 數學式有錯（表格顯示 11302%、129832%，但 DFR 譜顯示 ~80%）。追下去發現是**一整類 stale-index bug**：`calcAge` 的回傳 list 在 v3.7.4 被 restructure（從 V3.4.1 archive 還原），但 AutoPipeline 幾個消費端還在用舊 layout 的 index。`step['age_result']` 就是 `calcAge` 回傳（59 元素），正確 layout：
`0 ³⁶m, 2 ³⁶Air, 4 ³⁶Ca, 6 ³⁷m, 8 ³⁷Ca, 10 ³⁸m, 16 ³⁹m, 18 ³⁹K, 22 ⁴⁰m, 24 ⁴⁰Ar*, 46 age(yr), 47 σ, 50 ⁴⁰Ar*/⁴⁰m`。

### 問題 + 修法（3 處，全是錯 index）

1. **⁴⁰Ar(r)% 試算（使用者回報）**：`_apply_ar36_scale` / `_ar36_neg_count` 用 `ar[10]` 當 ⁴⁰Ar_m,但 `ar[10]` 是 **³⁸Ar_m**。pct = ⁴⁰Ar*/³⁸Ar_m → 爆成上萬 %。修成 `ar[22]`（⁴⁰Ar_m）。
2. **Summary banner 的 Normal/Inverse Isochron**：`_update_isochron_stats` 用 `ar[2,4,6,8,10]` 當 ³⁶/³⁷/³⁸/³⁹/⁴⁰ measured,但那是 ³⁶Air/³⁶Ca/³⁷m/³⁷Ca/³⁸m。**這就是 trapped ⁴⁰/³⁶ 一直跑出負值（-25.8/-41.6）、isochron age 178/202 Ma 的原因。** 修成 `ar[0,6,10,16,22]`。
3. **Ca/K 欄**：`populate` 與 AgeCalc_summary.csv 匯出用 `ar[23]`（=⁴⁰Ar_m_std≈0）→ 整欄顯示 0.000。修成跟 datum/DFA 一致：`(³⁹K · PR(39/37ca)) / ³⁷Ca` = `(ar[18]·consts[0])/ar[8]`。

### 影響

- **plateau / total fusion age 不受影響**（age 走 `ar[24]`=⁴⁰Ar*、`ar[18]`=³⁹K、`ar[46]`=age，本來就正確；NO.65 plateau 仍 ~8.9 Ma）。
- 受影響的是 **Summary banner 的 isochron age + trapped ratio、Ca/K 欄、³⁶ 試算的 ⁴⁰Ar(r)%**,這些之前都是垃圾。
- DFR 譜、datum CSV 的 ⁴⁰Ar(r)%（走 `ar[50]` / datum row[19]）本來就對，沒動。

### 驗證（NO.65 1100°C 實檔 calcAge）

- ⁴⁰Ar(r)%：fixed 75.40%（bug 10276%）= `ar[50]`×100 一致
- Ca/K：fixed 0.4317（bug ~0.000006）,1000/1100/1200/1300 = 0.31/0.43/0.19/0.69，落在 DFA 譜範圍
- isochron 40/36：fixed 1426（bug 10.5）
- [ ] **使用者需重跑 pipeline** 確認 Summary banner 的 isochron trapped ⁴⁰/³⁶ 變回大氣附近、Ca/K 欄出現非零值

### 檔案改動

- `AutoPipeline.py`：`_apply_ar36_scale`/`_ar36_neg_count`（ar[10]→ar[22]）、`_update_isochron_stats`（ar[2,4,6,8,10]→ar[0,6,10,16,22]）、`populate`/`_export` Ca/K（ar[23]→(ar[18]·consts[0])/ar[8]）。
- `.work/.app_info.txt`：3.8.73 → 3.8.74

---

## V3.8.73（2026-06-04）— ³⁶Ar 試算/敏感度：標出 net ³⁶(atm) 變負（⁴⁰Ar(r)%>100%）

使用者回報：³⁶Ar net 已經壓到最小，k>1 會讓 net ³⁶ 變負（age 當然變老），希望試算功能要告訴他 ³⁶ 是不是負的。

net atmospheric ³⁶ < 0 ⟺ ⁴⁰Ar* > ⁴⁰Ar_m ⟺ ⁴⁰Ar(r)% > 100%（大氣校正變成「加」而不是「減」，非物理）。三處加旗標：

- 新增 `_ar36_neg_count(k)`：算某 k 下有幾階 net ³⁶(atm)<0。
- `_apply_ar36_scale`（試算按鈕）：統計 `n_neg36`，label 加紅字警告「⚠ N 階 net ³⁶(atm)<0 → ⁴⁰Ar(r)%>100%」，該階 ⁴⁰Ar(r)% 格子標紅，整條 label 變紅。
- `_show_ar36_spectrum_dialog`（Age 譜敏感度）：拉桿每動算 `_ar36_neg_count(k)`，plateau 標籤後面接警告、變紅。
- `populate`（主結果表）：⁴⁰Ar(r)% > 100% 的格子直接標紅 + tooltip，不用開試算就看得到 baseline 是否已經越界。

### 檔案改動

- `AutoPipeline.py`：`_ar36_neg_count`；`_apply_ar36_scale` / 敏感度 dialog / `populate` 加旗標。
- `.work/.app_info.txt`：3.8.72 → 3.8.73

### 驗證 checklist

- [x] `_ar36_neg_count` 邏輯測試：k=1 算到已 >100% 的階、k>1 越界數增加、k<1 歸零
- [x] compile 過
- [ ] GUI：試算與敏感度視窗在 net ³⁶<0 時出現紅字警告；主表 ⁴⁰Ar(r)%>100% 標紅

---

## V3.8.72（2026-06-04）— HOTFIX：v3.8.71 害 pyADR 開不起來

v3.8.71 把「Show diagrams」勾選框放在 Plot Controls（建構早段），但勾選框 `setChecked(self._diag_visible.get(...))` 讀的 `self._diag_visible` 卻在後面的 diagram grid loop 才定義 → `AgeCalcPage.__init__` 直接 `AttributeError`。AgeCalcPage 在 app 啟動時就被 AutoPipelineWindow 建構，所以整個 pyADR 開不起來。

修法：把 `self._diag_visible` / `self._diag_order` 的定義**移到 Plot Controls 勾選框之前**，grid loop 內的重複定義移除。`_on_diag_toggle`/`_relayout_diagram_grid` 在建構早期被 `setChecked` 觸發時，`_relayout` 用 `getattr(self,'_dg_grid',None)` 早退，安全。

### 檔案改動

- `AutoPipeline.py`：`_diag_visible`/`_diag_order` 定義位置上移；grid loop 去除重複定義。
- `.work/.app_info.txt`：3.8.71 → 3.8.72

### 驗證 checklist

- [x] compile 過；靜態確認 `_diag_visible` 定義（4738）早於使用（4752），`_dg_grid`/`_dframes` 在 grid 段定義、`_relayout` 有 getattr 早退保護
- [ ] GUI：pyADR 正常開啟，AgeCalc Plot Controls 的 Show diagrams 勾選正常

備註：sandbox 無法跑 Qt GUI（Qt 在此環境 C-level crash），此版靠靜態分析 + compile 驗證定義順序；請重開 pyADR 確認。
